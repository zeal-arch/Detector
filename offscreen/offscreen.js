...
async function handleMergeAndDownload(msg, sendResponse) {
  ...
  // We also pass these sizes into _doMerge so the progress bar pre-knows
  // the total (video + audio) from the very first progress update.
  let estimatedSize = 0;
  let knownVideoLength = 0;
  let knownAudioLength = 0;
  try {
    const videoHead = await fetch(videoUrl, { method: "HEAD" });
    knownVideoLength = parseInt(
      videoHead.headers.get("content-length") || "0",
      10,
    );
    const audioHead = await fetch(audioUrl, { method: "HEAD" });
    knownAudioLength = parseInt(
      audioHead.headers.get("content-length") || "0",
      10,
    );
    estimatedSize = knownVideoLength + knownAudioLength;
    console.log(
      `[OFFSCREEN] Merge size estimate: video=${(knownVideoLength / 1024 / 1024).toFixed(1)}MB audio=${(knownAudioLength / 1024 / 1024).toFixed(1)}MB total=${(estimatedSize / 1024 / 1024).toFixed(1)}MB`,
    );
  } catch (e) {
    // HEAD may fail (YouTube returns 200 only for GET), continue anyway
 ...
 */
async function _doMerge(msg, sendResponse, knownVideoLength = 0, knownAudioLength = 0) {
  const { videoUrl, audioUrl, filename, mergeId } = msg;
  ...
  try {
    ...
    // Pre-warm totals from HEAD request sizes so the progress bar
    // shows the correct combined total immediately (not just audio ~75MB)
    const combinedMB = knownVideoLength + knownAudioLength > 0
      ? ` (${((knownVideoLength + knownAudioLength) / 1024 / 1024).toFixed(0)} MB total)`
      : "";
    sendProgress(progressKey, "download", `Downloading audio + video${combinedMB}...`, 0);

    // --- Combined progress tracker ---
    // Maps the entire merge process to a single 0→100% bar:
    //   Download: 0–70%  |  Merge: 70–90%  |  Finalize: 90–100%
    // Pre-seed totals from HEAD request so combined size is shown from byte 0.
    let audioDL = 0,
      audioTotal = knownAudioLength;  // pre-seeded from HEAD
    let videoDL = 0,
      videoTotal = knownVideoLength;  // pre-seeded from HEAD
