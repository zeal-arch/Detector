(function () {
    ...
    function extractFromPage() {
        ...
        for (const script of scripts) {
            ...
            const streamMatch = content.match(/"(?:stream_url|streamUrl|url)"\s*:\s*"([^"]+\.(m3u8|mp3|aac)[^"]*)"/gi);
            if (streamMatch) {
                for (const match of streamMatch) {
                    const urlMatch = match.match(/"([^"]+\.(m3u8|mp3|aac)[^"]*)"/);
                    if (urlMatch) {
                        const url = urlMatch[1];
                        videos.push({
                            url: url,
                            type: url.includes('.m3u8') ? 'hls' : 'mp3',
                            quality: url.includes('.m3u8') ? 'auto' : 'audio',
        ...
        for (const audio of audioElements) {
            ...
            if (src && !src.startsWith('blob:')) {
                videos.push({
                    url: src,
                    type: src.includes('.m3u8') ? 'hls' : 'mp3',
                    quality: 'audio',
                    title: document.title
                });
            }
    ...
    log('iHeartRadio specialist initialized');
})();
