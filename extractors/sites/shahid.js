(function () {
    ...
    function extractFromPage() {
        ...
        for (const script of scripts) {
            ...
            const dashMatch = content.match(/["']([^"']+\.mpd[^"']*)/gi);
            if (dashMatch) {
                for (const match of dashMatch) {
                    const url = match.replace(/^['"]|['"]$/g, '');
                    videos.push({
                        url: url,
                        type: 'dash',
            ...
            const playbackMatch = content.match(/"(?:playbackUrl|streamUrl|url)"\s*:\s*"([^"]+)"/gi);
            if (playbackMatch) {
                for (const match of playbackMatch) {
    ...
    log('Shahid specialist initialized');
})();
