(function () {
    ...
    function extractFromPage() {
        ...
        for (const script of scripts) {
            ...
            const clipMatch = content.match(/"(?:clipUrl|videoUrl|url)"\s*:\s*"([^"]+)"/gi);
            if (clipMatch) {
                for (const match of clipMatch) {
                    const urlMatch = match.match(/"([^"]+)"/g);
                    if (urlMatch && urlMatch[1]) {
                        const url = urlMatch[1].replace(/"/g, '');
                        if (url.includes('.m3u8') || url.includes('.mp4') || url.includes('clip')) {
                            videos.push({
                                url: url,
    ...
    log('Pluralsight specialist initialized');
})();
