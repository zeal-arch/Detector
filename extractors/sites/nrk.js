(function () {
    'use strict';
    console.log('[NRK Specialist] Loaded on:', window.location.href);
    ...
    function extractVideo() {
        try {
            ...
            const _idMatch = window.location.pathname.match(/\/(?:program|serie)\/([^/]+)/);

            const scripts = document.querySelectorAll('script');
            for (const script of scripts) {
                const content = script.textContent || '';

                const dataMatch = content.match(/"playbackUrl"\s*:\s*"([^"]+)"/);
                if (dataMatch) {
            ...
            console.error('[NRK] Extraction error:', err);
        }
    }
