class EastMojoExtractor extends BaseExtractor {
  constructor() {
    super("EastMojo");
  }

  static get URL_PATTERNS() {
    return [{ match: /^https?:\/\/(www\.)?eastmojo\.com\// }];
  }
  ...
  async extract() {
    ...
    videos.push(...this._extractFromWebStories());
  ...
  _extractFromMediaElements() {
    ...
    for (const video of document.querySelectorAll("video")) {
      if (video.src)
        videos.push({
          url: video.src,
          quality: this._getQuality(video.src),
      ...
      for (const src of video.querySelectorAll("source")) {
        if (src.src)
          videos.push({
            url: src.src,
            quality: this._getQuality(src.src),
  ...
  _extractFromArticleLinks() {
    ...
    for (const link of article.querySelectorAll("a[href]")) {
      ...
      const fbMatch = href.match(/facebook\.com\/.*\/videos?\//);
      if (fbMatch) {
        videos.push({
          url: href,
          quality: "embed",
          format: "embed",
          title: this._getTitle() + " (Facebook)",
        });
        continue;
      }
      ...
      if (/\.(?:mp4|m3u8|mp3|m4a|webm|ogg)(\?|$)/i.test(href)) {
        ...
        videos.push({
          url: href,
          quality: isAudio ? "audio" : this._getQuality(href),
  ...
  _extractFromWebStories() {
    const videos = [];

    for (const ampVideo of document.querySelectorAll("amp-video")) {
      const src =
        ampVideo.getAttribute("src") ||
        ampVideo.querySelector("source")?.getAttribute("src");
      if (src)
        videos.push({
          url: src,
          quality: "auto",
          format: this._extFromUrl(src, "mp4"),
          title: this._getTitle() + " (Story)",
        });
    }
    return videos;
  }
...
if (typeof globalThis !== "undefined") {
  globalThis.EastMojoExtractor = EastMojoExtractor;
}
