(function () {
    ...
    function extractFromPage() {
        ...
        for (const script of scripts) {
            ...
            const streamMatch = content.match(/"streaming_url"\s*:\s*"([^"]+)"/);
            if (streamMatch) {
                videos.push({
                    url: streamMatch[1],
                    type: 'mp3',
            ...
            const mp3Match = content.match(/"(?:url|source|src)"\s*:\s*"([^"]+\.mp3[^"]*)"/gi);
            if (mp3Match) {
                for (const match of mp3Match) {
    ...
    log('Audiomack specialist initialized');
})();
