(function () {
    ...
    function extractFromPage() {
        ...
        for (const script of scripts) {
            ...
            const manifestMatch = content.match(/"(?:manifest|manifestUrl|url)"\s*:\s*"([^"]+)"/gi);
            if (manifestMatch) {
                for (const match of manifestMatch) {
            ...
            const videoMatch = content.match(/"(?:src|source|video_url)"\s*:\s*"([^"]+\.(mp4|webm)[^"]*)"/gi);
            if (videoMatch) {
    ...
    log('Nebula specialist initialized');
})();
