(function () {
    ...
    function extractFromPage() {
        ...
        for (const script of scripts) {
            ...
            const audioMatch = content.match(/"(?:playback_url|download_url|url)"\s*:\s*"([^"]+)"/gi);
            if (audioMatch) {
                for (const match of audioMatch) {
                    const urlMatch = match.match(/"([^"]+)"/g);
                    if (urlMatch && urlMatch[1]) {
                        const url = urlMatch[1].replace(/"/g, '');
                        if (url.includes('.mp3') || url.includes('audio') || url.includes('episode')) {
                            videos.push({
                                url: url,
                                type: 'mp3',
                                quality: 'audio',
        ...
        for (const script of ldJson) {
            try {
                ...
                for (const item of items) {
                    if (item['@type'] === 'PodcastEpisode' || item['@type'] === 'AudioObject') {
                        const url = item.contentUrl || item.audio?.contentUrl;
                        if (url) {
                            videos.push({
                                url: url,
                                type: 'mp3',
    ...
    log('Spreaker specialist initialized');
})();
