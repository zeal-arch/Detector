(function () {
    ...
    function extractFromPage() {
        ...
        for (const script of scripts) {
            ...
            const videoInfoMatch = content.match(/window\.videoInfo\s*=\s*(\{[^;]+\})/);
            if (videoInfoMatch) {
                try {
                    const videoInfo = JSON.parse(videoInfoMatch[1]);
                    if (videoInfo.playUrls) {
                        for (const url of videoInfo.playUrls) {
                            videos.push({
                                url: url,
            ...
            const mp4Match = content.match(/"(?:url|playUrl|videoUrl)"\s*:\s*"([^"]+\.mp4[^"]*)"/gi);
            if (mp4Match) {
    ...
    log('AcFun specialist initialized');
})();
