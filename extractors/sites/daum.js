(function () {
    ...
    function extractFromPage() {
        ...
        for (const script of scripts) {
            ...
            const playerMatch = content.match(/kakao\.player\s*\(\s*(\{[^}]+\})/);
            if (playerMatch) {
                try {
                    ...
                    if (config.url) {
                        videos.push({
                            url: config.url,
                            type: config.url.includes('.m3u8') ? 'hls' : 'mp4',
                            quality: 'auto',
            ...
            const videoMatch = content.match(/"(?:videoUrl|playUrl|url)"\s*:\s*"([^"]+)"/gi);
            if (videoMatch) {
    ...
    log('Daum specialist initialized');
})();
