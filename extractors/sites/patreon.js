(function () {
    ...
    function extractFromPage() {
        ...
        for (const script of scripts) {
            ...
            const videoMatch = content.match(/"(?:video_url|download_url|url)"\s*:\s*"([^"]+\.(mp4|webm)[^"]*)"/gi);
            if (videoMatch) {
            ...
            const vimeoMatch = content.match(/player\.vimeo\.com\/video\/(\d+)/);
            if (vimeoMatch) {
                videos.push({
    ...
    log('Patreon specialist initialized');
})();
