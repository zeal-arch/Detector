...
var CENCDecryptor = (function () {
  ...
  async function decryptAesCbc(key, iv, data) {
    ...
    ivPadded.set(iv.subarray(0, Math.min(iv.length, 16)));
  ...
  async function decryptFullSample(cryptoKey, scheme, iv, sample) {
    if (scheme === "cbc1" || scheme === "cbcs") {
      ...
      const encryptedPart = sample.subarray(0, blockCount * 16);
      const remainder = sample.subarray(blockCount * 16);
  ...
  async function decryptSubsampleSample(
    ...
    for (const sub of subsamples) {
      // Copy clear bytes
      if (sub.clearBytes > 0) {
        output.set(sample.subarray(offset, offset + sub.clearBytes), outOffset);
      ...
      if (sub.protectedBytes > 0) {
        const protectedData = sample.subarray(
          offset,
    ...
    if (offset < sample.length) {
      output.set(sample.subarray(offset), outOffset);
    }
  ...
  async function decryptCbcsPattern(
    ...
    while (offset < data.length) {
      ...
      if (offset + cryptBytes <= data.length) {
        const encrypted = data.subarray(offset, offset + cryptBytes);
        ...
        output.set(data.subarray(offset), offset);
        break;
      }
      offset += cryptBytes;
      ...
      output.set(data.subarray(offset, skipEnd), offset);
      offset = skipEnd;
    }
