  ...
  const compat = {
    ...
    async executeScript(tabId, opts) {
      if (compat.hasScripting) {
        ...
        if (opts.func) {
          // MV2 limitation: opts.func is serialized via toString(), so it
          // must NOT close over outer variables (they won't be available in
          // the target page context). opts.args are JSON-serialized and
          // passed as arguments to the function.
          if (opts.args && opts.args.length > 0) {
            config.code = `(${opts.func.toString()}).apply(null, ${JSON.stringify(opts.args)})`;
          } else {
