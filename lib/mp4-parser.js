...
var MP4Parser = (function () {
  ...
  function* iterBoxes(buf, start = 0, end = undefined) {
    ...
    while (offset < limit) {
      ...
      box.data = buf.subarray(box.dataOffset, offset + box.size);
      yield box;
  ...
  function extractPSSH(buf) {
    ...
    for (const box of psshBoxes) {
      ...
      // flags = data[1..3]
      const systemId = WidevineProto.toHex(data.subarray(4, 20));
      ...
      if (version === 1) {
        ...
        for (let i = 0; i < kidCount; i++) {
          kidList.push(WidevineProto.toHex(data.subarray(offset, offset + 16)));
          offset += 16;
        }
      }
      ...
      offset += 4;
      const psshData = data.subarray(offset, offset + dataSize);

      // Reconstruct the full PSSH box for base64 encoding
      const start = box.dataOffset - box.headerSize;
      const end = start + box.size;
      if (start < 0 || end > buf.length) {
        // Malformed box — skip to avoid out-of-bounds read
        continue;
      }
      const fullBox = buf.subarray(start, end);
  ...
  function parseTenc(buf) {
    ...
    const defaultKID = WidevineProto.toHex(data.subarray(8, 24));

    let constantIV = null;
    if (perSampleIVSize === 0 && data.length > 24) {
      const constantIVSize = data[24];
      constantIV = data.subarray(25, 25 + constantIVSize);
    }
  ...
  function parseSenc(buf, perSampleIVSize) {
    ...
    for (let i = 0; i < sampleCount && offset < data.length; i++) {
      // Bounds check: ensure enough bytes for IV
      if (offset + perSampleIVSize > data.length) return null;
      const iv = data.subarray(offset, offset + perSampleIVSize);
      offset += perSampleIVSize;

      const subsamples = [];
      if (hasSubsamples) {
        // Bounds check: ensure enough bytes for subsampleCount
        if (offset + 2 > data.length) return null;
        const subsampleCount = view.getUint16(offset, false);
        offset += 2;
        for (let j = 0; j < subsampleCount; j++) {
          // Bounds check: ensure enough bytes for clearBytes(2) + protectedBytes(4)
