...
function renderVideo(info) {
  ...
  if (videoOnly.length > 0 && audioOnly.length > 0) {
    ...
    for (const f of videoOnly) {
      ...
      // Show combined video + audio total so the user sees what will actually download
      const vBytes = parseInt(f.contentLength || 0);
      const aBytes = parseInt(best.contentLength || 0);
      const combinedBytes = vBytes + aBytes;
      const sizeLabel = combinedBytes > 0
        ? ` · ${formatSize(combinedBytes)} (V+A)`
        : f.contentLength ? ` · ${formatSize(f.contentLength)} video` : "";
      optionsHtml += `<option value="${val}"${disabled}>${q} · ${codec}${fps}${sizeLabel}</option>`;
    }
